...
public class SearchSongRecyclerViewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        switch (holder.getItemViewType()) {
            case TYPE_TRACK:
                ...
                // YouTube songs use artist field directly; SoundCloud songs use User object
                if (song.getArtist() != null && !song.getArtist().isEmpty()) {
                    ...
                    eachSongViewHolder.artistName.setText(R.string.unknown_artist);
                }
    ...
    public int getItemCount() {
        int count = 0;
        if (hasTrackHeader) count = FOOTER_TRACK + (hasTrackFooter ? 1 : 0);
        if (hasUserHeader) count = FOOTER_USER + (hasUsersFooter ? 1 : 0);
        if (hasPlaylistHeader) count = FOOTER_PLAYLIST + (hasPlaylistFooter ? 1 : 0);
        // If no sections have headers, just count songs
        if (!hasTrackHeader && !hasUserHeader && !hasPlaylistHeader) return 0;
        return count;
    }
    ...
    public void changeUserData(List<User> users) {
        ...
        if (hasTrackFooter) HEADER_USER = FOOTER_TRACK + 1;
        else HEADER_USER = FOOTER_TRACK;
    ...
    public void changePlaylistData(List<Playlist> playlists) {
        ...
        if (hasUsersFooter) HEADER_PLAYLIST = FOOTER_USER + 1;
        else HEADER_PLAYLIST = FOOTER_USER;
