...
public class FetchSongFilesTask extends AsyncTask<File, Void, Void> {
    ...
    private ArrayList<File> getSongs(File root) {
        ...
        if (files == null) return songsList;
        try {
            ...
            for (File singleFile : files) {
                if (singleFile.isDirectory() && !singleFile.isHidden()) {
                    ...
                    if ((singleFile.getName().endsWith(".mp3") || singleFile.getName().endsWith(".m4a"))
                        ...
                        Song song = getSongMetaData(singleFile);
                        if (song != null) {
                            sqliteDbAdapter.addNewSong(song);
                        }
                    }
                }
            }
    ...
    public Song getSongMetaData(File file) {
        ...
        if (title == null) {
            try {
                ...
                Log.e(TAG, "Failed to release metadata retriever for " + file.getAbsolutePath(), e);
            }
            return null;
        }
